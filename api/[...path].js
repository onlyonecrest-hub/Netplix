...
export default async function handler(req, res) {
  try {
    ...
    if (pathname.startsWith("/api/vidsrc-domains")) return handleDomains(req, res);
    if (pathname.startsWith("/api/tmdb")) return handleTmdb(req, res);
    if (pathname.startsWith("/api/omdb")) return handleOmdb(req, res);
    if (pathname.startsWith("/api/poster")) return handlePoster(req, res);
    if (pathname.startsWith("/api/vidsrc-latest")) return handleLatest(req, res);
    if (pathname.startsWith("/api/health")) return sendJson(res, 200, { ok: true, ...envStatus() });
