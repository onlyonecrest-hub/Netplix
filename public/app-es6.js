...
function card(item) {
  ...
  const progress = item.progressLabel ? `<span class="progress-badge">${escapeHtml(item.progressLabel)}</span>` : "";
  return `
...
function wireGlobal() {
  ...
  $("#prevButton").addEventListener("click", () => navigateWatch(-1));
  $("#nextButton").addEventListener("click", () => navigateWatch(1));
  ...
  document.querySelectorAll(".server-option").forEach((button, index) => {
    button.addEventListener("click", () => {
      ...
      renderServerOptions();
      if (state.selected) {
        const source = embedUrl(state.selected);
        $("#playerFrame").src = source;
      }
    });
  });
  ...
  window.addEventListener("beforeunload", saveDeviceState);
}
...
function getNavigationItems() {
  const suggestions = state.rows.get("suggestions")?.items || [];
  if (suggestions.length > 1) return suggestions;
  return [...state.itemCache.values()].filter((item) => item.mediaType === state.selected?.mediaType);
}

function getCurrentWatchIndex(items) {
  if (!state.selected) return -1;
  return items.findIndex((item) => item.mediaType === state.selected.mediaType && String(item.id) === String(state.selected.id));
}

async function navigateWatch(direction) {
  const items = getNavigationItems();
  if (!items.length || !state.selected) return;
  const currentIndex = getCurrentWatchIndex(items);
  const nextIndex = ((currentIndex + direction + items.length) % items.length + items.length) % items.length;
  const nextItem = items[nextIndex];
  if (nextItem) openWatch(nextItem.mediaType, nextItem.id);
}

function toggleZoom() {
  const shell = document.querySelector(".watch-shell");
  if (!shell) return;
  shell.classList.toggle("is-zoomed");
  const button = $("#zoomButton");
  button.textContent = shell.classList.contains("is-zoomed") ? "Normal View" : "Zoom";
}

function renderServerOptions() {
  document.querySelectorAll(".server-option").forEach((button, index) => {
    const serverName = state.domains[index % state.domains.length] || "source";
    button.textContent = `${index === 0 ? "Full HD" : index === 1 ? "Backup HD" : "Fast Stream"} - ${serverName}`;
    button.classList.toggle("is-active", index === state.domainIndex);
  });
  const active = state.domains[state.domainIndex % state.domains.length] || "source";
